...
def DifferenceAverageOdds(y_pred, y_real, sensitivecat, outcome, privileged, unprivileged, labels):
    ...
    print('FPR unprivileged: ' + str(FP_unpriv/(FP_unpriv+TN_unpriv)) + '(0.0647) Debiased paper')
    print('FPR privileged: ' + str(FP_priv/(FP_priv+TN_priv)) + '(0.0701) Debiased paper')
    print('FNR unprivileged: ' + str(FN_unpriv/(FN_unpriv+TP_unpriv)) + '(0.04458) Debiased paper')
    print('FNR privilged: ' + str(FN_priv/(FN_priv+TP_priv)) + '(0.4349) Debiased paper')
